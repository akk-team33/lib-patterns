 ...
 */
public class Crazy<T> implements Supplier<T> {

    private volatile Supplier<T> backing;

    private Crazy(final Supplier<? extends T> initial) {
     ...
     */
    public static <T> Crazy<T> init(final Supplier<? extends T> initial) {
        return new Crazy<>(initial);
    }
