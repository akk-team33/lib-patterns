...
final class Equation<T> {
    ...
    static <T> Equation<T> of(final Class<T> type, final Function<T, ?> function) {
        return new Equation<>(type, function);
    }

    final boolean equals(final T left, final Object right) {
        return left == right || (type.isInstance(right) && function.apply(left)
                                                                   .equals(function.apply(type.cast(right))));
    }

    final int hashCode(final T instance) {
        return function.apply(instance).hashCode();
    }

    final String toString(final T instance) {
